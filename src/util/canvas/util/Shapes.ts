...
export class Shapes {
  ...
  private _rotatePoint(
    ...
    const x = Math.cos(angle) * (pointX - originX) - Math.sin(angle) * (pointY - originY) + originX;
    const y = Math.sin(angle) * (pointX - originX) + Math.cos(angle) * (pointY - originY) + originY;
    return { x, y };
  }
  ...
  private _makeGradient(colors: string[], rotate?: number): CanvasGradient {
    ...
    if (rotate) {
      ...
      const rotatedStart = this._rotatePoint(originX, originY, startX, startY, rotate);
    ...
    const gradient = this._canvasContext.createLinearGradient(startX, startY, endX, endY);
  ...
  private _implementOptions(options: any, closePath = true) {
    if (typeof options?.lineColor === 'string') this._canvasContext.strokeStyle = options.lineColor;
    ...
    if (typeof options?.fillColor === 'string') this._canvasContext.fillStyle = options.fillColor;
  ...
  public polygon(points: Array<{ x: number; y: number }>, options?: IPolygonOptions) {
    this._canvasContext.beginPath();
    ...
    for (let i = 0; i < points.length; i++) {
      ...
      if (options?.rounded) this._canvasContext.quadraticCurveTo(points[i].x, points[i].y, xc, yc);
